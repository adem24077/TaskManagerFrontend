...
export function storeTaskState(task) {
  localStorage.setItem("task", JSON.stringify(task));
}

export function loadRentalState() {
  const defaultState = { createdDate: "" };
  const taskStateInStorage = localStorage.getItem("task");
  if (!taskStateInStorage) return defaultState;
  try {
    return JSON.parse(taskStateInStorage);
  } catch {
    return defaultState;
  }
}
